...
try:
    ...
    from gen_named_pipe.pro.config.template_dir import TemplateDir
...
class ReadTemplate(FileChecking, TemplateDir):
    ...
    GEN_VERBOSE = 'GEN_NAMED_PIPE::PRO::READ_TEMPLATE'
    ...
    def read(self, config, np_type, verbose=False):
        '''
        ...
        error, status = checker.check_params([
            ('dict:config', config), ('str:np_type', np_type)
        ])
        ...
        for template_file, module_file in zip(templates, modules):
            ...
            template_file_path = '{0}{1}/{2}'.format(
                self.template_dir, np_type, template_file
            )
