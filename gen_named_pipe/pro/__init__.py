...
'''
 ...
 Info
     Defined class NamedPipe with attribute(s) and method(s).
...
try:
    from gen_named_pipe.pro.config import ProConfig
    from gen_named_pipe.pro.config.pro_name import ProName
    from gen_named_pipe.pro.read_template import ReadTemplate
    from gen_named_pipe.pro.write_template import WriteTemplate
...
class NamedPipe(FileChecking, ProConfig, ProName):
    '''
        Defined class NamedPipe with attribute(s) and method(s).
        Generate module file generator_test.py by template and parameters.
        It defines:
            ...
            :methods:
                ...
                | __str__ - dunder method for NamedPipe.
    '''

    GEN_VERBOSE = 'GEN_NAMED_PIPE::PRO::GEN_SETUP'
    ...
    def __init__(self, verbose=False):
        ...
        verbose_message(
            NamedPipe.GEN_VERBOSE, verbose, 'init generator'
        )
        self.__reader = ReadTemplate(verbose=verbose)
        self.__writer = WriteTemplate(verbose=verbose)
        project_structure = '{0}{1}'.format(
            dirname(realpath(__file__)), NamedPipe.PRO_STRUCTURE
        )
    ...
    def gen_setup(self, pro_name, verbose=False):
        ...
        verbose_message(
            NamedPipe.GEN_VERBOSE, verbose,
            'generating named pipe modules for', pro_name
        )
        if bool(self.config):
