...
Info
    Defines class GenNamedPipe with attribute(s) and method(s).
...
class GenNamedPipe(CfgCLI):
    '''
        ...
        It defines:
            ...
            :methods:
                | __init__ - Initials GenNamedPipe constructor.
                | process - Processes and runs operations.
    '''

    _GEN_VERBOSE: str = 'GEN_NAMED_PIPE'
    _CONFIG: str = '/conf/gen_named_pipe.cfg'
    _LOG: str = '/log/gen_named_pipe.log'
    _LOGO: str = '/conf/gen_named_pipe.logo'
    _OPS: List[str] = ['-n', '--name', '-t', '--type', '-v', '--verbose']

    def __init__(self, verbose: bool = False) -> None:
        '''
            Initials GenNamedPipe constructor.
        ...
        if self.tool_operational:
            self.add_new_option(
                ...
                help='generate driver (provide project name)'
            )
            self.add_new_option(
                self._OPS[2], self._OPS[3], dest='type',
                help='driver type (posix)'
            )
            self.add_new_option(
        ...
        if self.tool_operational:
            try:
                ...
                if exists(f'{getcwd()}/{str(getattr(args, "name"))}'):
                    ...
                    return status
                gen: NamedPipe = NamedPipe(getattr(args, 'verbose') or verbose)
                try:
                    print(
                        " ".join([
                            f'[{self._GEN_VERBOSE.lower()}]',
